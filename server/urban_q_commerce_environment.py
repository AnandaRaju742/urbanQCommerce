...
class UrbanQCommerceEnvironment(Environment):
    ...
    def __init__(self, task_level: str = "medium", max_steps: int = 50): # Accept max_steps
        self._state = UrbanQCommerceState()
        self._cargo, self._pos_id = 50, 0
        self._max_steps = max_steps # Use the provided max_steps
        self.task_level = task_level

    def reset(self, seed=None, episode_id=None, **kwargs) -> UrbanQCommerceObservation:
        # Allow max_steps to be overridden during reset if provided in kwargs
        self._max_steps = kwargs.get("max_steps", self._max_steps)
    ...
    def step(self, action: UrbanQCommerceAction, **kwargs) -> UrbanQCommerceObservation:
        ...
        if done:
            raw_reward = 1.0 - (self._state.total_sla_breaches * 0.05)
            obs.reward = max(0.001, min(0.999, raw_reward))
        else:
            obs.reward = 0.0
    ...
    def _obs(self, message: str) -> UrbanQCommerceObservation:
        ...
        return UrbanQCommerceObservation(
            ...
            active_nodes=nodes, steps_remaining=self._max_steps - self._state.step_count, message=message
        )
